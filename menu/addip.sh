...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Scvpsss/izin/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Scvpsss/izin/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Scvpsss/izin/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
#tarap=$( curl -sS https://raw.githubusercontent.com/Scvpsss/izin/main/ip | grep $MYIP )
#if [ $tarap = $MYIP ]; then
...
cat << EOF >> /etc/crontab
# IPREGBEGIN_EXP
1 0 * * * root /usr/bin/vps # delete expired IP VPS License
# IPREGEND_EXPIP
EOF
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/Scvpsss/scvip/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
